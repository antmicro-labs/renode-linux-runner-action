...
def setup_python(child: px.spawn):
    ...
    try:
        ...
        run_cmd(child, "#", f"pip install {' '.join([f'/var/packages/{package}' for package in downloaded_packages])} --no-index --no-deps --no-build-isolation --root-user-action=ignore")
